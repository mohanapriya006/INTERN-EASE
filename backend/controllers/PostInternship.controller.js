// import Internship from "../models/PostInternship.model.js";  error cause you exporeted postinternship in model
...
export const createInternship = async (req, res) => {
    try {
        ...
        const { 
            title, company, location, stipend, type, 
            startDate, endDate, description, 
            requirements, applicationDeadline 
        } = req.body;
        ...
        if (deadline >= start) {
            return res.status(400).json({ 
                success: false, 
                message: "Application deadline should be before start date." 
            });
        }
...
export const getAllInternships = async (req, res) => {
  try {
    ...
    if (stipendMin) {
      ...
      // parsing of the stipend field if it contains text like "₹25K/month"
      filter.stipend = { $gte: stipendMin };
    }
...
export const getInternshipById = async (req, res) => {
  try {
    const internship = await Internship.findById(req.params.id)
      .populate("postedBy", "name organization email phone");
...
export const getCompanyInternships = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Server error while fetching company internships",
      error: error.message
    });
  }
};
...
export const updateInternship = async (req, res) => {
  try {
    ...
    // Check if the logged-in company owns this internship
    if (internship.postedBy.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        success: false,
        message: "Unauthorized: You can only update your own internship listings"
      });
    }
...
export const deleteInternship = async (req, res) => {
  try {
    ...
    if (internship.postedBy.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        success: false,
        message: "Unauthorized: You can only delete your own internship listings"
      });
    }
